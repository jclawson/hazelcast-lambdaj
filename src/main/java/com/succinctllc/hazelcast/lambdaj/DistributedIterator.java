...
public class DistributedIterator<F, T, TYPE, VAL> implements Iterator<T> {
    ...
    public DistributedIterator(AbstractTypeProvider<TYPE, VAL> provider,
            ConverterDTO<TYPE, T> converter, Set<Member> members) {
    ...
    public TYPE localNext() {
        try {
            if (localValueIterator != null) return localValueIterator.next();
            else return null;
    ...
    private VAL fetchVal() {
        try {
            ...
            Future<VAL> future = Hazelcast.getExecutorService().submit(
                    new FetchTask<TYPE, VAL>(provider));
            return future.get();
        ...
        if (type == null) {
            while (type == null && hasNext()) {
                ...
                if (value instanceof Collection) {
                    ...
                    if (localValueIterator.hasNext()) type = localValueIterator.next();
                } else {
                    return converter.convert((TYPE) value);
                }
            }
        }
        if (type != null) return converter.convert(type);
        else return null;
    }
