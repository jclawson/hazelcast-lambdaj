        ...
        while (!(clazz.getGenericSuperclass() instanceof ParameterizedType)) {
            ...
            if (clazz == null)
                throw new RuntimeException(
                        "Unable to determine managed class, did you forget to provide the T template parameter to AbstractDao?");
        }

        Type type = ((ParameterizedType) clazz.getGenericSuperclass())
                .getActualTypeArguments()[0];

        cls = (Class<TYPE>) (type instanceof ParameterizedType ? ((ParameterizedType) type)
                .getRawType() : type);
    }
