...
public class HazelcastLambdaj {
    ...
    public static <F, T, TYPE, VAL> Iterator<T> convertIterator(
            AbstractTypeProvider<TYPE, VAL> accessor, Set<Member> members,
            ConverterDTO<TYPE, T> converter) {
        return new DistributedIterator<F, T, TYPE, VAL>(accessor, converter, members);
    }

    public static <T, TYPE, VAL> List<T> collect(AbstractTypeProvider<TYPE, VAL> accessor,
            T argument) {
        Set<Member> members = Hazelcast.getCluster().getMembers();
        ArgumentConverter<TYPE, T> converter = new ArgumentConverter<TYPE, T>(argument);
        ArgumentConverterDTO<TYPE, T> converterDto = new ArgumentConverterDTO<TYPE, T>(converter);
    ...
    private static Number typedSum(Object iterable, Class<?> numberClass) {
        ...
        return iterator.hasNext() ? aggregate(iterator, getSumAggregator(iterator.next()))
                : typedZero(numberClass);
    }
