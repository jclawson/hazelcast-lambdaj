...
public class InvocationSequenceDTO implements Serializable, Inflatable<InvocationSequence> {
    ...
    public InvocationSequenceDTO(InvocationSequence invocationSequence) {
		try {
			...
			Field f1 = InvocationSequence.class.getDeclaredField("lastInvocation");
			f1.setAccessible(true);
			this.lastInvocation = new InvocationDTO((Invocation) f1.get(invocationSequence));
		} catch (Exception e) {
	...
	public InvocationSequence inflate() {
    	...
    	try {
	    	Field f = InvocationSequence.class.getDeclaredField("lastInvocation");
